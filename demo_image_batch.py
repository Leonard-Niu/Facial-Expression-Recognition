...
parser.add_argument('--show_resultimg', default=False, help='show the result img')
...
if __name__ == '__main__':
    img_name = os.listdir(arg.input_folder)
    img_name.sort(key=lambda i: int(re.match(r'(\d+)', i).group()))
    for img in img_name:
        print('The image path is % s' % img)
        image_predict(img)
